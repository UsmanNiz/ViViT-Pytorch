...
class Block(nn.Module):
    def __init__(self, config, vis, droplayer_p=0.0):
    ...
    def get_drop_pattern(self, x, training):
        """Generate per-sample drop pattern following Scenic implementation.
        ...
        Returns:
            Drop pattern tensor of shape (batch_size, 1, 1) or 0.0
        """
        if training and self.droplayer_p > 0.0:
            # Per-sample dropping: shape (batch_size, 1, 1) for broadcasting
            shape = (x.shape[0], 1, 1)
            drop_pattern = torch.bernoulli(
                torch.full(shape, self.droplayer_p, device=x.device, dtype=x.dtype)
            )
            return drop_pattern
        return 0.0
    ...
    def forward(self, x):
        ...
        # Apply stochastic depth to attention skip connection
        drop_pattern = self.get_drop_pattern(x, self.training)
        x = x * (1.0 - drop_pattern) + h

        # MLP block with stochastic depth
        h = x
        x = self.ffn_norm(x)
        x = self.ffn(x)
        
        # Apply stochastic depth to MLP skip connection
        drop_pattern = self.get_drop_pattern(x, self.training)
        x = x * (1.0 - drop_pattern) + h
        return x, weights
...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        stochastic_droplayer_rate = config.transformer.get("stochastic_droplayer_rate", 0.0)
        
        for lyr in range(num_layers):
            # Linear drop rate scheduling: 0 at first layer, stochastic_droplayer_rate at last
            droplayer_p = (lyr / max(num_layers - 1, 1)) * stochastic_droplayer_rate
            layer = Block(config, vis, droplayer_p=droplayer_p)
